...
const NewCollectionsWomen = () => {
  const [newCollectionWomen, setNewCollectionWomen] = useState([]);

  useEffect(() => {
    fetch("http://localhost:4000/newcollections?category=women")
      .then((response) => response.json())
      .then((data) => setNewCollectionWomen(data));
  }, []);

  return (
    <section className='bg-primary'>
      <div className='max_padd_container py-12 xl:py-28 xl:w-[88%]'>
        <h3 className='h3 text-center'>New Collections for Women</h3>
        <hr className='h-[3px] md:w-1/2 mx-auto bg-gradient-to-l from-transparent via-black to-transparent mb-16' />
        <div className='grid grid-col-1 xs:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-6'>
          {newCollectionWomen.slice(0, 4).map((item) => {
