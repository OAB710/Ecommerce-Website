import React from 'react';
import { FaSearch } from 'react-icons/fa';
import { Link } from 'react-router-dom';

const ProductCard = ({ product }) => {
  const imageUrl = product.image.startsWith('http') ? product.image : `http://localhost:4000/${product.image}`;

  return (
    <div className='rounded-x1 overflow-hidden shadow-lg'>
      <div className='relative flexCenter group overflow-hidden transition-all duration-100'>
        <Link to={`/product/${product.id}`} className='h-12 w-12 bg-white rounded-full flexCenter absolute top-1/2 bottom-1/2 !py-2 z-20 scale-0 group-hover:scale-100 transition-all duration-700'>
          <FaSearch className='scale-125 hover:rotate-90 transition-all duration-200' />
        </Link>
        <img onClick={() => window.scrollTo(0, 0)} src={imageUrl} alt='productImage' className='w-full block object-cover group-hover:scale-110 transition-all duration-1000' />
      </div>
      <div className='p-4 overflow-hidden'>
        <h4 className='my-[6px] medium-16 line-clamp-2 text-gray-30'>{product.name}</h4>
        <div className='flex gap-5'>
          <div className='bold-16'>{product.new_price}.00</div>
          <div className='text-secondary bold-16 line-through'>{product.old_price}.00</div>
        </div>
        <p className='text-gray-500 text-sm'>{product.shortDescription}</p>
        <p className='text-gray-800 text-sm'>Tag: {product.tags}</p>
      </div>
    </div>
  );
};

export default ProductCard;